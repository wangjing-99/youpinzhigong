<template>
  <div :style="isDialog?'position:fixed;':'position:relative;'">
    <div class="container">
      ...
      <a :href="'/pages/index/index?shop='+shop" open-type="redirect" hover-class="none" class="section-nav-item">
        <div class="s-l"><img :src="shopInfo.shoplogo"></div>
        <div class="s-c">
          <h3>{{shopInfo.shopname}}的小店</h3>
          <p>{{shopInfo.shopinfo}}</p>
        </div>
        <div class="s-r">
          进入店铺
          <img class="i" src="/static/images/address_right.png" background-size="cover"/>
      ...
      <!--<div class="related-goods" v-if="likeProduct.length">-->
        <!--<div class="b">-->
          <!--<div class="item" v-for="(item, index) of likeProduct" :key="item.id" :data-index="index">-->
            <!--<navigator open-type="redirect" :url="'/pages/goods/goods?id=' + item.id">-->
    ...
    <div class="bottom-btn">
      ...
      <a :href="'/pages/index/index?shop='+shop" open-type="reLaunch" hover-class="none" class="l l-home">
        <div class="box">
          <icon class="icon"></icon>
          <div class="text">首页</div>
        </div>
      </a>
      <a href="/pages/cart/cart" open-type="redirect" hover-class="none" class="l l-cart">
        <div class="box">
      ...
      <block v-if="goodsInfo.product_status != 1 || goodsInfo.stocks < 1">
        <div class="c c-c">加入购物车</div>
        <div class="r r-r">立即购买</div>
      </block>
      <block v-else>
        <div class="c" @click="addToCart">加入购物车</div>
        <div class="r" @click="addToCart">立即购买</div>
      </block>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getGoodsInfo() {
        util.showLoading()
        const res = await api.getGoodsDetail({id: this.gid});
        util.hideLoading()
        if (res.code === 0) {
