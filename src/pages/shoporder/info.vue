  ...
  <view class="container">
    <view class="order-state">
      ...
      <view class="state-close" v-if="orderInfo.status === '-10'||orderInfo.status === '-20'">
        ...
        <view class="close">
          <view>{{orderInfo.status_str.name}}</view>
        </view>
      </view>
      ...
      <view class="state-success"
        ...
        <view class="state">
          ...
          <view class="success">{{orderInfo.status_str.name}}</view>
        </view>
      ...
      <div class="status-waiting" v-if="orderInfo.status === '15'">
        ...
        <div class="title">
          <h3>{{orderInfo.status_str.name}}</h3>
          <p>剩 <b>{{orderInfo.status_str.expire}}</b> 自动关闭</p>
        </div>
      </div>
    ...
    <view class="order-detail">
      ...
      <view class="goods-item" v-for="(orders,indx) in orderInfo.product" :key="indx"
            @click.stop="goGoods(orders.product_id_code)">
  ...
  export default {
    ...
    methods: {
      ...
      async getShopOrderInfo() {
        util.showLoading()
        const res = await api.getShopOrderInfo({
          no: this.orderId
        })
        util.hideLoading()
        if (res.code === 0) {
          this.orderInfo = res.data.info
          this.like = res.data.likeproduct
