      ...
      <scroll-view v-if="data.length>0" @scrolltolower="lower" class="order-scroll" scroll-y="true"
        ...
        <view v-for="(item, index) in data" :key="index" class="order-detail">
          ...
          <view v-for="(product, idx) in item.product" :key="idx" class="goods-item"
                @click.stop="goGoods(product.id,item.order_no)">
            ...
            <view class="goods-info">
              ...
              <view class="goods-spac">
                ...
                <view>
                  ...
                  <view v-if="product.return_state" class="earn">
                    赚￥{{product.commission_amount}}
                  </view>
                  <view v-if="product.action.length>0" @click.stop="actionorder(product.action)" :class="product.action.name=='退款中' || product.action.name=='退款成功' || product.action.name=='退货中'?'goods-action act-red':'goods-action'">
                    {{product.action.name}}
                  </view>
                </view>
              </view>
  ...
  export default {
    ...
    methods: {
      ...
      async getShopOrderList() {
        ...
        util.showLoading()
        const res = await api.getShopOrderList({
          page: this.page,
          status: this.status
        })
        util.hideLoading()
      ...
      goGoods(id, orderNo) {
        if (id) {
          util.navigate('/pages/shoporder/info?id=' + id + '&no=' + orderNo)
        } else {
          util.showToast('没有id')
        }
      },
