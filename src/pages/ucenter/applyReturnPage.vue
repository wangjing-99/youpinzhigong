  ...
  <view class="container">
    ...
    <view class="item">
      ...
      <view class="item-right">
        <picker class="r-l" :value="receiveindex" :range="receive" @change="changeReceive" mode="selector">
          <view class="picker"> {{receive[receiveindex]}}</view>
    ...
    <div class="photo-list">
      <h3>上传凭证</h3>
      <div @click="uploadPhoto('a')" class="photo">
        <img class="update-img" v-if="card_img_a.url" :src="card_img_a.url" alt="">
        <div class="updateImg"></div>
      </div>
      <div @click="uploadPhoto('b')" class="photo">
        <img class="update-img" v-if="card_img_b.url" :src="card_img_b.url" alt="">
        <div class="updateImg"></div>
      </div>
      <div @click="uploadPhoto('c')" class="photo">
        <img class="update-img" v-if="card_img_c.url" :src="card_img_c.url" alt="">
        <div class="updateImg"></div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      async applyReturnPage(param) {
        util.showLoading()
        const res = await api.applyReturnPage(param)
        util.hideLoading()
        if (res.code === 0) {
          ...
          success: function (res) {
            wx.uploadFile({
              ...
              success: function (res) {
                ...
                if (res.code === 0) {
                  _this['card_img_' + name] = res.data[0]
                  util.showToast('上传成功')
                } else {
                  util.showToast(res.msg)
                }
      ...
      async submit() {
        ...
        const isReceiveGoods = this.receiveindex === 0 ? 1 : 0
        const backMoney = this.oldRefundAomunt
        const reason = this.reason[this.reasonindex]
        const note = this.note
        const itemId = this.data.product.id
        const price = this.refundAomunt
        const pathA = this.card_img_a.path
        const pathB = this.card_img_b.path
        const pathC = this.card_img_c.path
        ...
        const res = await api.applyReturn({
          ...
          card_img_b: pathB,
          card_img_c: pathC,
          is_receive_goods: isReceiveGoods,
          note: note,
          item_id: itemId,
          t_price: price
        })
        if (res.code === 0) {
          util.redirect('/pages/ucenter/returnInfo?service_id=' + res.data.id)
        } else {
          util.showToast(res.msg)
        }
      }
