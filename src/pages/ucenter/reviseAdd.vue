  ...
  <view class="container">
    <view class="add-address">
      <view class="add-form">
        ...
        <view class="form-item">
          <input class="input" @input="bindinputMobile" :value="address.mobile" placeholder="请输入手机号码"/>
        </view>
        <view @click="isDialog=true" class="form-item">
          <input class="input" :value="address.areas_name" disabled="true" placeholder="省、市、区"/>
    ...
    <region v-if="address.id"  :isDialog="isDialog" @completeRegion="completeRegion"></region>
...
<script>
  ...
  export default {
    ...
    methods: {
      completeRegion(res) {
        if (res) {
          this.address.areas_name = res.fullRegion
      ...
      async getAddressList() {
        util.showLoading()
        const res = await api.getUpdateaddres({
          id: this.address.id
        })
        util.hideLoading()
        if (res.code === 0) {
          this.address = {
            ...
            is_default: parseInt(res.data.is_default),
            cityids: res.data.province + ',' + res.data.city + ',' + res.data.area
          }
        } else {
      ...
      async saveAddress() {
        ...
        const res = await api.updateAddress({
          ...
          accept_phone: address.mobile,
          cityids: address.cityids,
          address: address.address,
          is_default: address.is_default
        });
        // console.log('保存地址,请求结果', res);
        if (res.code === 0) {
          wx.navigateBack({datafdsaf: 1});
        }
      }
    },
    onUnload() {
